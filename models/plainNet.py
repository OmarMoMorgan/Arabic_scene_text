...
class cnnBlock(nn.Module):
    ...
    def forward(self,x):
        ...
        return self.act2(self.BN2(self.cnn2(out)))
...
class plainNet(nn.Module):
    def __init__(self,
        ...
        self.base = nn.Sequential(nn.BatchNorm2d(f1),
                                  nn.ReLU())
        self.stage1= nn.Sequential(*[cnnBlock(f1,f1)
                                    for _ in range(layersPerStage[0])])
        layers= [cnnBlock(f1,f2,2)]
        layers.extend([cnnBlock(f2,f2) for _ in range(layersPerStage[1]-1)])
        self.stage2 = nn.Sequential(*layers)

        layers= [cnnBlock(f2,f3,2)]
        layers.extend([cnnBlock(f3,f3) for _ in range(layersPerStage[2]-1)])
